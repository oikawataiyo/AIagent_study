...
# パスを強制的に追加（main_feedback.py のディレクトリの親ディレクトリをパスに設定）
...
def select_model():
    models = ("GPT-4", "Claude 3 Sonnet", "Claude 3.5 Sonnet" "GPT-3.5 (not recommended)")
    ...
    if prompt := st.chat_input(placeholder="法人で契約することはできるの?"):
        ...
        if st.session_state['first_question']:
            if cache_content := cache.search(query=prompt):
                with st.chat_message("assistant"): st.write(f"(cache) {cache_content}")
